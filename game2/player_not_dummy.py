...
class NotDummyPlayer(AbstractPlayer):

    def __init__(self, name, start_balance):
        super().__init__(name, start_balance)
        print("%s: Hi, im %s, a not dummy player at all and im ready to play" % (name, name))

    def make_bet(self, cards, hands, min_pot, enemy_money, enemy_rise):
        start = time.time()
        decision_factor = np.average(self.hands)
        self.hands.append(hands)

        if math.isnan(decision_factor) or hands > decision_factor:
            try:
                bet = random.randint(min_pot, self.balance)
            except ValueError:
                bet = min_pot  # TODO fixme it should not happened
        else:
            diff = decision_factor - hands
            bet = min_pot + round(diff * 10)
